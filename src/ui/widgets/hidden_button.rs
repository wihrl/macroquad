...
pub struct HiddenButton<'a> {
    position: Option<Vec2>,
    size: Option<Vec2>,
    content: UiContent<'a>,
}

impl<'a> HiddenButton<'a> {
    pub fn new<S>(content: S) -> HiddenButton<'a>
    where
        S: Into<UiContent<'a>>,
    {
        HiddenButton {
...
impl Ui {
    pub fn hidden_button<'a, P: Into<Option<Vec2>>, S: Into<UiContent<'a>>>(
        &mut self,
        position: P,
        label: S,
    ) -> bool {
        HiddenButton::new(label).position(position).ui(self)
    }
}
